...
And while we're listing things Valve merely *allows*: Steam is the storefront where Denuvo became normal. Valve's own DRM is famously mild. It's also famously optional, layered under whatever a publisher wants to staple on top. The store that could have drawn a line decided lines are the customer's problem. You can now buy a single-player game on Steam that phones an Austrian DRM server before letting you play offline. On Steam. The platform whose whole pitch was convenience beating piracy.
...
The part that makes this post annoying to write: the other column is *long*.

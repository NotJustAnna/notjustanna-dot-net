...
It gets a network. Not access to the host's network, but *a* network: its own interface, its own IP, wired through a bridge into private networks with other guests. Docker is running a virtual switch. It's the same box you'd click together in Proxmox, minus the clicking.
